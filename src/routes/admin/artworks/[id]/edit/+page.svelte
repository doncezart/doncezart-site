<script>
	...
	let { data, form } = $props();
	let displayMode = $state(data.artwork.displayMode || 'single');
	let carouselDirection = $state(data.artwork.carouselDirection || 'horizontal');
	let selectedFiles = $state([]);
	let uploading = $state(false);
	let selectedCategory = $state(data.artwork.category || '');
...
<form method="POST" enctype="multipart/form-data" use:enhance={() => { uploading = true; return async ({ update }) => { uploading = false; await update(); }; }} class="artwork-form">
	<label>
		Title *
		<input type="text" name="title" value={data.artwork.title} required />
	</label>

	<label>
		Description
		<textarea name="description" rows="3">{data.artwork.description ?? ''}</textarea>
	</label>
		...
		<label>
			...
			<select name="subcategory">
				...
				{#each filteredSubs as sub}
					<option value={sub.slug} selected={data.artwork.subcategory === sub.slug}>{sub.name}</option>
				{/each}
			</select>
		</label>
	</div>
	...
	<!-- Current images -->
	{#if data.images.length > 0 && selectedFiles.length === 0}
		<div class="current-images">
			<p class="label-text">Current images ({data.images.length})</p>
			<div class="image-strip">
				{#each data.images as img, i (img.id)}
					<div class="strip-item">
						<img src={img.thumbnailUrl || img.imageUrl} alt="Image {i + 1}" />
						{#if data.artwork.displayMode === 'before-after'}
							<span class="pos-label">{i === 0 ? 'Before' : 'After'}</span>
	...
	<label class="file-label">
		{selectedFiles.length > 0 ? 'New images (will replace current)' : 'Replace images (leave empty to keep current)'}
		<input
			type="file"
			name="images"
			...
			<div class="tag-grid">
				{#each data.tags as t}
					<label class="tag-check">
						<input type="checkbox" name="tags" value={t.id} checked={data.currentTagIds.includes(t.id)} />
						{t.name}
					</label>
				{/each}
			</div>
	...
	<label>
		...
		<select name="case_study_id">
			...
			{#each data.caseStudies as cs}
				<option value={cs.id} selected={data.artwork.caseStudyId === cs.id}>{cs.title}</option>
			{/each}
		</select>
	</label>
