...
- Card generator v11: **drop shadow moved onto the card element itself** (it can never lag behind the corners/size again — padding, radius and export-size changes update it in the same render tick; the preview wrapper's size is now derived from the config instead of measured, so nothing "snaps back" and export drags are twitch-free); **preview pane restored to 440px** (fits the tallest default cards with no scrollbars); with **zero padding the thumbnail radius locks to the container radius** — no more thin background slivers between the two corner arcs; **Wide Split finally matches the spec** — the thumbnail renders at its full 16:9 aspect filling its column exactly, the card's height equals that natural height, and the 2/3 text column is precisely double the thumbnail's area (ratio slider still adjusts the balance); the split **title defaults to 1 line** so the text fits the natural height; the obsolete **Split wideness slider was removed** (its flattening conflicted with the fixed natural-height spec; per-layout defaults re-applied via the existing touched-flags, now extended to title lines)
