...
- Card generator v12: **the drop-shadow stays glued to the card** — the leftover square wrapper shadow (`box-shadow` on `.preview-scaled`) was removed, so only the card's own radius-following shadow remains (verified: exactly one shadow in the preview, on `.ycard` at the container radius); **preview pane restored to 480px = the controls rail's height** (harness asserts pane 480 = rail 480) with the scale cap raised 0.36 → 0.5 so short cards no longer render tiny; **Wide Split now matches the spec** — description ON by default, the global **Text size ×** multiplier (0.5–1.5, new slider) defaults to 0.5 for split (title 28px, avatar 43px at 1280 — the thumbnail visually dominates), and the card stays at its natural 245px height with description on (verified: no crop, thumbnail stays 16:9 always); **Hero title defaults to 0.8×** via a per-layout `defaultTitleScale` applied on layout switch (the Title size slider visibly reads 0.8); per-layout defaults for titleScale/textScale/description join the touched-flags system (manual tuning sticks across layout switches); harness: +5 checks (rail-height equality, split description-on, split natural height with desc, split 0.5× text, hero 0.8× title)
