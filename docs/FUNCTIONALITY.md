...
- **Key functions / components:** `YouTubeCard.svelte` (6-layout renderer — Classic, Wide Split, Stacked, Hero, Underlay, Compact; metrics scaled from YouTube's 360px feed reference; container size 720–1920, split wideness, per-row gaps (thumb/column/text), card radius, outer-frame padding, hero scrim opacity, verified-badge colors, date format), `yt-config.js` (config model, palettes, layout metadata), `ExportBar.svelte` (html2canvas capture with mount-out for true-size export, file type PNG/JPEG/WebP + Download/Copy to clipboard, full-width rows), `SnapSlider.svelte` (notch slider with snap + mechanical-keyboard clack, numeric readout with min/max clamp and the unit rendered inside the box on the left), dismissible API-fallback notice showing the exact `dataError` from the server. Page layout: two short top columns (Layout buttons only | Style — colors row + font row), preview beside a 1-column Controls rail (same height as the preview) hosting every slider, Content band (preview width) + Export container below; preview pane has a top-right background picker (checkerboard, palette solids, custom color/image); the tool containers scroll smoothly into view after a link loads.
