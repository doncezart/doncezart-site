...
- **Key functions / components:** `YouTubeCard.svelte` (4-layout renderer, metrics scaled from YouTube's 360px feed reference; container size 720–1920, per-row gaps (thumb/column/text), card radius, outer-frame padding, hero scrim opacity), `yt-config.js` (config model, palettes, layout metadata), `ExportBar.svelte` (html2canvas capture with mount-out for true-size export, format/size, clipboard), `SnapSlider.svelte` (notch slider with snap + WebAudio tick), dismissible API-fallback notice shown in oEmbed mode.
